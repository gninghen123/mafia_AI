...
#pragma mark - Database Reset Methods

- (void)resetSymbolDatabase:(void (^)(BOOL success))completion {
    NSLog(@"DataHub: Starting symbol database reset...");
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        BOOL success = NO;
        
        @try {
            // 1. Delete all Symbol entities from Core Data
            NSFetchRequest *fetchRequest = [Symbol fetchRequest];
            NSArray<Symbol *> *allSymbols = [self.mainContext executeFetchRequest:fetchRequest error:nil];
            
            NSLog(@"DataHub: Found %lu symbols to delete", (unsigned long)allSymbols.count);
            
            for (Symbol *symbol in allSymbols) {
                [self.mainContext deleteObject:symbol];
            }
            
            // 2. Clear in-memory arrays
            [self.symbols removeAllObjects];
            
            // 3. Clear symbol data cache
            @synchronized(self.symbolDataCache) {
                [self.symbolDataCache removeAllObjects];
            }
            
            // 4. Clear market data caches related to symbols
            @synchronized(self.quotesCache) {
                [self.quotesCache removeAllObjects];
            }
            @synchronized(self.historicalCache) {
                [self.historicalCache removeAllObjects];
            }
            @synchronized(self.companyInfoCache) {
                [self.companyInfoCache removeAllObjects];
            }
            
            // 5. Save Core Data context
            [self saveContext];
            
            success = YES;
            NSLog(@"DataHub: Symbol database reset completed successfully");
            
        } @catch (NSException *exception) {
            NSLog(@"DataHub: Error during symbol database reset: %@", exception.reason);
            success = NO;
        }
        
        // 6. Send notifications on main thread
...
- (void)resetWatchlistDatabase:(void (^)(BOOL success))completion {
    NSLog(@"DataHub: Starting watchlist database reset...");
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        BOOL success = NO;
        
        @try {
            // 1. Delete all Watchlist entities from Core Data
            NSFetchRequest *fetchRequest = [Watchlist fetchRequest];
            NSArray<Watchlist *> *allWatchlists = [self.mainContext executeFetchRequest:fetchRequest error:nil];
            
            NSLog(@"DataHub: Found %lu watchlists to delete", (unsigned long)allWatchlists.count);
            
            for (Watchlist *watchlist in allWatchlists) {
                [self.mainContext deleteObject:watchlist];
            }
            
            // 2. Clear in-memory array
            [self.watchlists removeAllObjects];
            
            // 3. Save Core Data context
            [self saveContext];
            
            success = YES;
            NSLog(@"DataHub: Watchlist database reset completed successfully");
            
        } @catch (NSException *exception) {
            NSLog(@"DataHub: Error during watchlist database reset: %@", exception.reason);
            success = NO;
        }
        
        // 4. Send notifications on main thread
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter] postNotificationName:DataHubWatchlistUpdatedNotification
                                                                object:self
                                                              userInfo:@{@"action": @"reset"}];
...
- (void)resetAlertDatabase:(void (^)(BOOL success))completion {
    NSLog(@"DataHub: Starting alert database reset...");
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        BOOL success = NO;
        
        @try {
            // 1. Delete all Alert entities from Core Data
            NSFetchRequest *fetchRequest = [Alert fetchRequest];
            NSArray<Alert *> *allAlerts = [self.mainContext executeFetchRequest:fetchRequest error:nil];
            
            NSLog(@"DataHub: Found %lu alerts to delete", (unsigned long)allAlerts.count);
            ...
            success = YES;
            NSLog(@"DataHub: Alert database reset completed successfully");
            
        } @catch (NSException *exception) {
            NSLog(@"DataHub: Error during alert database reset: %@", exception.reason);
            success = NO;
        }
        
        // 4. Send notifications on main thread
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter] postNotificationName:@"DataHubAlertsUpdatedNotification"
                                                                object:self
                                                              userInfo:@{@"action": @"reset"}];
...
- (void)resetConnectionDatabase:(void (^)(BOOL success))completion {
    NSLog(@"DataHub: Starting connection database reset...");
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        BOOL success = NO;
        
        @try {
            // 1. Delete all StockConnection entities from Core Data
            NSFetchRequest *fetchRequest = [StockConnection fetchRequest];
            NSArray<StockConnection *> *allConnections = [self.mainContext executeFetchRequest:fetchRequest error:nil];
            
            NSLog(@"DataHub: Found %lu connections to delete", (unsigned long)allConnections.count);
            
            for (StockConnection *connection in allConnections) {
                [self.mainContext deleteObject:connection];
            }
            ...
            success = YES;
            NSLog(@"DataHub: Connection database reset completed successfully");
            
        } @catch (NSException *exception) {
            NSLog(@"DataHub: Error during connection database reset: %@", exception.reason);
            success = NO;
        }
...
- (void)resetAllDatabases:(void (^)(BOOL success))completion {
    NSLog(@"DataHub: Starting NUCLEAR reset of ALL databases...");
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        __block BOOL allSuccess = YES;
        __block NSInteger pendingResets = 4;
        
        void (^checkCompletion)(BOOL success) = ^(BOOL success) {
            if (!success) {
                allSuccess = NO;
            }
            ...
            if (pendingResets == 0) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    NSLog(@"DataHub: NUCLEAR reset completed with result: %@", allSuccess ? @"SUCCESS" : @"PARTIAL FAILURE");
                    
                    // Final notification
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"DataHubNuclearResetCompleted"
                                                                        object:self
        ...
        // Reset all databases in parallel
        [self resetSymbolDatabase:checkCompletion];
        [self resetWatchlistDatabase:checkCompletion];
        [self resetAlertDatabase:checkCompletion];
        [self resetConnectionDatabase:checkCompletion];
    });
}
@end
